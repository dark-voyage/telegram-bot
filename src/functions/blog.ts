...
const ctxMenuText =
  `<b>Blog contents written by Sokhibjon</b>` +
  `\n` +
  `\n` +
...
composer.command("blog", async (ctx: Context): Promise<void> => {
  ...
  const request: Response = await fetch("https://orzklv.uz/api/blog");
  const contents: Post[] = await request.json();

  for (const post of pager<Post>(contents, 1)) {
    keyboard.url(post.title, `https://orzklv.uz/blog/${post.slug}`).row();
  }
